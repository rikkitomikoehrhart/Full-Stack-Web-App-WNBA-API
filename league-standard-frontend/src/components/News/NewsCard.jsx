...
function NewsCard({ article }) {
    const { bookmarksIDs, toggleNewsBookmarks } = useBookmarks();
        ...
        <>
            <div className="card m-4 shadow-sm" key={article.id}>
                ...
                <div className='mt-auto'>
                    <button className='btn btn-link p-0 border-0 position-absolute' onClick={() => toggleNewsBookmarks(article.id)} style={{ background: 'none', top: '10px', left: '10px', zIndex: 1 }}>
                        {bookmarksIDs.has(article.id) ? (
